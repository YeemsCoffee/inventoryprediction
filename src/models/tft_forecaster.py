...
class TFTForecaster:
    ...
    def prepare_data_for_product(
        ...
        scaled_series = series.map(lambda x: scaler.fit_transform(x.reshape(-1, 1)).reshape(-1, 1, 1))
    ...
    def train_tft(
        ...
        try:
            ...
            pred_unscaled = prediction.map(
                lambda x: scaler.inverse_transform(x.reshape(-1, 1)).reshape(-1, 1, 1)
            )
    ...
    def predict(
        ...
        scaled_series = original_series.map(
            lambda x: scaler.transform(x.reshape(-1, 1)).reshape(-1, 1, 1)
        )
